 ...
 * runner's contract: it owns its try/catch and NEVER throws â€” a failing
 * or slow LLM call must not affect the webhook's 200 to Meta.
 *
 * Eligibility gates (any â†’ silent no-op):
 ...
 *
 * The 24h WhatsApp session window is inherently open here â€” we're
 * reacting to a customer message that just landed â€” so no separate
...
export async function dispatchInboundToAiReply(
  ...
  try {
    ...
    // Deterministic, user-configured responders win over the LLM â€” the
    // caller already excludes messages a Flow consumed. Message-level
    // automations (`new_message_received` / `keyword_match`) are
    // dispatched independently for this same inbound and may send their
    // own reply, so if the account has any active one we stand down to
    // avoid double-texting the customer. (Relationship triggers like
    // `first_inbound_message` don't count â€” they're not per-message
    // auto-responders.)
    ...
    // owner's key past the provider's rate limit. Over the limit â†’ skip
    ...
    if (!acctLimit.success) {
      console.warn(
        `[ai auto-reply] account ${accountId} hit the per-account rate limit â€” skipping this inbound.`,
      )
      return
    }
    ...
    // Logged regardless of handoff â€” the provider call happened either
    // way.
    void logAiUsage(db, {
      accountId,
      conversationId,
    ...
    if (handoff || !text) {
      ...
      // The model can't (or shouldn't) answer â€” stop auto-replying on
      // this thread and hand it to a human. We (a) pause the bot here
      // (sticky until re-enabled), (b) route the conversation to the
      // configured handoff agent â€” null leaves it in the shared queue â€”
      // and (c) leave a short internal note so whoever picks it up has
      // context. Assigning fires the `on_conversation_assigned` trigger,
      // which notifies the agent.
      const handoffReason = inferHandoffReason(messages)
      ...
      // isn't already owned â€” never stomp an existing human assignment.
      if (config.handoffAgentId && !conv.assigned_agent_id) {
        update.assigned_agent_id = config.handoffAgentId
      }
      await db.from('conversations').update(update).eq('id', conversationId)
      try {
        await moveOpenDealToQualified(db, {
          accountId,
          userId: configOwnerUserId,
          contactId,
          conversationId,
        ...
        console.warn('[ai auto-reply] pipeline handoff stage sync skipped:', err)
      }
      return
    }
    ...
    // skip the send. (We consume a slot slightly before the send lands â€”
    ...
    if (claimErr) {
      ...
      // deploy issue â€” e.g. `claim_ai_reply_slot` not EXECUTE-able by the
